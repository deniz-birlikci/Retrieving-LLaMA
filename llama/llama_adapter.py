...
class LLaMA_adapter(nn.Module):
    ...
    def forward_retrieval(self, tokens, imgs):
        visual_query = self.forward_visual(imgs)
        ...
        h = self.llama.norm(h)
        output = self.llama.out_retrieval(h[:, -1, :])
        return output

    def forward_retrieval_text(self, tokens):
        _bsz, seqlen = tokens.shape
...
def load(name, llama_dir, llama_type="7B", 
         device="cuda" if torch.cuda.is_available() else "cpu", 
         download_root='ckpts', max_seq_len=512, phase="finetune",
         clip_model='ViT-L/14'
         ):
    if name in _MODELS:
